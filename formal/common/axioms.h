...
/*
 * The TRUSTED surface of the formal model, plus the shared logic
 * functions (to_signed, op_bits) and the Qed-proved ground lemmas.
 * Every `axiom` here states a fact WP's SMT encoding cannot derive
 * (bitwise AND, variable shifts) and MUST be proved over its full
 * instantiation domain by CBMC before use:
 *
 *     cd ../axiom_validation && cbmc validate_specs_axioms.c
 *
 * must print VERIFICATION SUCCESSFUL. A false axiom silently
 * invalidates every WP proof in this tree.
 */
...
// Ground values of RTE_LEN2MASK(opsz) for the two supported widths:
// the macro's term is lsr(allones, 64 - opsz), and under the mask case
// split the provers face lsr(allones, 32) / lsr(allones, 0) — ground
// lsr terms they cannot evaluate. Needed by every call-site requires
// of the form `v <= RTE_LEN2MASK(opsz, uint64_t)` (eval_umax_bits /
// eval_uand_max chain) and the width goals built on it.
lemma len2mask_32: (0xFFFFFFFFFFFFFFFF >> 32) == 0xFFFFFFFF;
lemma len2mask_64: (0xFFFFFFFFFFFFFFFF >> 0) == 0xFFFFFFFFFFFFFFFF;

// Ground unfoldings of op_bits on the two supported masks: with these
// e-nodes present, `v <= RTE_LEN2MASK(op_bits(msk))`-shaped goals close
// by congruence with len2mask_32/64 after the mask case split, instead
// of the provers having to instantiate the op_bits definition and
// resolve its ite inside a large PO.
lemma op_bits_32: op_bits(0xFFFFFFFF) == 32;
lemma op_bits_64: op_bits(0xFFFFFFFFFFFFFFFF) == 64;

// Ground all-ones-shape facts for the four mask constants: axioms
// guarded by `(m & (m+1)) == 0` (the all-ones test) can only fire if
// that ground land evaluates, which provers cannot do — Qed folds
// constant lands, so state them as lemmas.
