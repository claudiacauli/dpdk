...
int main(void)
{
	...
	uint64_t wx, wy;
#if defined(BMC_UMAX)
	wx = rd.u.max; wy = rs.u.min;
#elif defined(BMC_UMIN)
	wx = rd.u.min; wy = rs.u.max;
#elif defined(BMC_SMAX)
	wx = (uint64_t)rd.s.max & msk; wy = (uint64_t)rs.s.min & msk;
#elif defined(BMC_SMIN)
	wx = (uint64_t)rd.s.min & msk; wy = (uint64_t)rs.s.max & msk;
#else
# error "define a BMC_* endpoint"
	...
	{ __int128 smax_w = (int64_t)(msk >> 1), smin_w = -(int64_t)(msk >> 1) - 1;
	  ...
	  __int128 dmax = (__int128)rd.s.max - (__int128)rs.s.min;
	  REQUIRE(smin_w <= dmin && dmin <= smax_w && smin_w <= dmax && dmax <= smax_w); }
  #endif
#endif
