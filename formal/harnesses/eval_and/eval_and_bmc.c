...
int main(void)
{
	...
	/* eval_alu always hands eval_and a fresh local copy of the source
	 * register, and the WP contract requires \separated(rd, rs) */
	const struct bpf_reg_val *prs = &rs;

	/* requires clauses of eval_and */
	...
	/* unsigned view: patterns are closed under AND */
	uint64_t ures = ux & uy;
	assert(rd.u.min <= ures && ures <= rd.u.max);          /* usound */

	/* signed view: AND the pattern of x with uy, sign-extend
	 * (mirrors eval_and_signed_soundness: to_signed(v & y, msk)) */
