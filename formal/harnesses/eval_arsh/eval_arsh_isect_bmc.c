/*
 * EXPERIMENT (Marat's intersection-soundness form): does eval_arsh's
 * usound/ssound hold WITHOUT range_agreement when the witness is a pattern
 * constrained to the INTERSECTION of both tracks? The signed-track
 * constraint to_signed(px) in rd.s is exactly the bound arsh's usound
 * computation needs (it shifts the signed reading of the pattern).
 *
...
int main(void)
{
	...
	/* INTERSECTION witness: pattern px in BOTH rd tracks; shift py in BOTH rs tracks */
	...
	REQUIRE(py < opsz);
	int64_t v = tos(px, msk);       /* signed reading of the pattern */

	/* intersection ssound: arithmetic shift of the canonical value */
	int64_t s_val = v >> py;
	assert(rd.s.min <= s_val && s_val <= rd.s.max);        /* ssound (isect) */

	/* intersection usound: same, re-encoded to a w-bit pattern */
	uint64_t ures = (uint64_t)(v >> py) & msk;
