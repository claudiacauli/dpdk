...
#if defined(BMC_UMAX)
	xv = rd.u.max; q = rs.u.min;
#elif defined(BMC_UMIN)
	xv = rd.u.min; q = rs.u.max;
#elif defined(BMC_SMAX)
	xv = (uint64_t)rd.s.max & msk; q = (rd.s.max < 0) ? rs.u.max : rs.u.min;
#elif defined(BMC_SMIN)
	xv = (uint64_t)rd.s.min & msk; q = (rd.s.min < 0) ? rs.u.min : rs.u.max;
#else
# error "define a BMC_* endpoint"
	...
	assert(repr(&od, xv, msk) && repr(&os, q, msk));
#if defined(BMC_UMAX)
	assert(res == rd.u.max);
#elif defined(BMC_UMIN)
	assert(res == rd.u.min);
