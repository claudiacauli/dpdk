/*
 * Intersection-soundness BMC harness for eval_divmod, matching the declared WP
...
int main(void)
{
	...
	struct bpf_reg_val *prs = &rs;   /* eval_divmod takes non-const rs */
	...
	REQUIRE(1 <= py);  /* 1 <= y (divisor) */
#if defined(BMC_DIV_ONLY)
	uint32_t op = BPF_DIV;
#elif defined(BMC_MOD_ONLY)
	uint32_t op = BPF_MOD;
#else
	uint32_t op = (uint32_t)nondet_int();
	REQUIRE(op == BPF_DIV || op == BPF_MOD);
#endif

	(void)opsz;
	eval_divmod(op, &rd, prs, msk);

	assert(range_ordering(&rd));                            /* ord */
	assert(range_within_width(&rd, msk));                  /* width */

	/* intersection usound: (op?x%y:x/y) covered by output u */
	uint64_t ures = (op == BPF_DIV) ? (px / py) : (px % py);
	assert(rd.u.min <= ures && ures <= rd.u.max);          /* usound (isect) */

	/* intersection ssound: to_signed(op?v%y:v/y) covered by output s */
	uint64_t sres = (op == BPF_DIV) ? (px / py) : (px % py);
