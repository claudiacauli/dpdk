...
int main(void)
{
	...
	/* BPF allows `lsh rX, rX`, but eval_alu always hands eval_lsh a
	 ...
	 * here would make the second shift read an already-shifted bound —
	 * a call pattern the real caller cannot produce. */
	const struct bpf_reg_val *prs = &rs;

	/* requires clauses of eval_lsh */
	...
	/* soundness witnesses cover in-range shift amounts only; for
	 * uy >= opsz the implementation widened to full width, which
	 * covers any oversized-shift machine semantics */
	REQUIRE(uy < opsz);

	uint64_t ushift = (ux << uy) & msk;
	assert(rd.u.min <= ushift && ushift <= rd.u.max);      /* usound */

	/* signed result: shift the w-bit pattern of x, mask, sign-extend */
	uint64_t sres = (((uint64_t)x & msk) << uy) & msk;
