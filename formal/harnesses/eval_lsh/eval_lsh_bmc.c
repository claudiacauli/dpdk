/*
 * Intersection-soundness BMC harness for eval_lsh, matching the declared WP
...
int main(void)
{
	...
	/* intersection ssound: to_signed((((v)&msk) << y) & msk) covered by output s */
	uint64_t sres = ((((uint64_t)px) & msk) << py) & msk;
