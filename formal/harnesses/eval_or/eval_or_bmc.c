...
int main(void)
{
	...
	/* eval_alu always hands eval_or a fresh local copy of the source
	 * register, and the WP contract requires \separated(rd, rs) */
	const struct bpf_reg_val *prs = &rs;

	/* requires clauses of eval_or */
	...
	/* concrete witnesses for the \forall of the two predicates: signed
	 * value x for the rd side, pattern uy for the rs side (both
	 * predicates quantify the rs operand over os.u), pattern ux for the
	 * unsigned track */
	...
	/* unsigned view: patterns are closed under OR */
	uint64_t ures = ux | uy;
	assert(rd.u.min <= ures && ures <= rd.u.max);          /* usound */

	/* signed view: OR the pattern of x with uy, mask to the w-bit
	 * result, sign-extend (mirrors eval_or_signed_soundness:
	 * to_signed((v | y) & msk, msk)). Unlike AND, OR does not self-bound
