...
echo
echo "=== SND (expect all PASS) — dispatcher VALUE soundness ==="
n=0
for entry in $OPS; do
	name=${entry%%:*}; code=${entry##*:}
	for w in 32 64; do
		cell "snd-$name$w" -DALL_FIXES -DBMC_OP="$code" -DBMC_"$w" \
			-DBMC_SND &
		n=$((n + 1)); [ $((n % 4)) -eq 0 ] && wait
	done
done
...
cell sanity-snd-add64 -DALL_FIXES -DBMC_OP=0x00 -DBMC_64 -DBMC_SND \
	-DBMC_SANITY &
wait
